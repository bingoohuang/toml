...
`
	...
	var val simple1
	_, err := Decode(testSimple, &val, WithPrefixMap(map[string]string{"age": "foo"}))
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = Decode(testSimple, &val, WithPrefixMap(map[string]string{"age": "bar"}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecodeErrors(t *testing.T) {
	for _, s := range []string{
		...
		// https://github.com/bingoohuang/toml/issues/155.
...
// Test for https://github.com/bingoohuang/toml/pull/166.
